//c++ -o pf175 PF175.cpp ../src/Event.cc ../src/TriggerMaker.cc `root-config --cflags --glibs`
...
int main(){
    ...
    for(int j = 0; j < klambdas.size(); j++){
        ...
        histos.at(j)->GetXaxis()->SetTitle("Online Calo Jets P_{T}^{1} (Gev)");
        if(j == 0){
            //SM configurations
        ...
        for(int i = 0; i < entries; i++){
            ...
            if(stop > 3 || stop < 0){//leading jet
                if(PFjets.size() > 0){
                    histos.at(j)->Fill(calo_pts.at(calo_pts.size()-1), ev.weight);
                }
            }
    ...
    for(int j = 0; j < klambdas.size(); j++){
        ...
        histos_cdf.at(j)->GetXaxis()->SetTitle("Online Calo Jets P_{T}^{1} (Gev)");
    ...
    TLine* l1 = new TLine(75., 0.001, 75., 1.2 );
    l1->SetLineStyle(kDashed);
    l1->SetLineColorAlpha(kGray, 0.7);

    TLine* l2 = new TLine(10., 0.9969, 610., 0.9969 );
    l2->SetLineStyle(kDashed);
    l2->SetLineColorAlpha(kGray, 0.7);

    TLine* l3 = new TLine(10., 0.9969, 610., 0.9969 );
    l3->SetLineStyle(kDashed);
    l3->SetLineColorAlpha(kGray, 0.7);

    TLine* l4 = new TLine(10., 0.9969, 610., 0.9969 );
    l4->SetLineStyle(kDashed);
    l4->SetLineColorAlpha(kGray, 0.7);

    TLine* l5 = new TLine(10., 0.9969, 610., 0.9969 );
