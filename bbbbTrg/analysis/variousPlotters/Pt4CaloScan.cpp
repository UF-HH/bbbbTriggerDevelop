//c++ -o calopt4scan Pt4CaloScan.cpp ../src/Event.cc ../src/TriggerMaker.cc `root-config --cflags --glibs`
...
int main(){
    ...
    for(int j = 0; j < klambdas.size(); j++){
        ...
        histos.push_back(new TH1F(legState.c_str(), legState.c_str(), 100, 10., 210));
        ...
        histos.at(j)->GetXaxis()->SetTitle("Online Calo Jets P_{T}^{4} (Gev)");
        if(j == 0){
            //SM configurations
        ...
        for(int i = 0; i < entries; i++){
            ...
            if(stop != 0){//only events that at least arrive to PFHT = 330 Cut
                if(Calojets.size() >= 4){
                    histos.at(j)->Fill(calo_pts.at(calo_pts.size()-4), ev.weight);
                }
            }
        }
        ...
        std::string y_title = "(1/N)#DeltaN / 2 GeV";
    ...
    c->Draw();
    c->SaveAs("Calopt4MatchedJets.pdf");
    ...
    for(int j = 0; j < klambdas.size(); j++){
        ...
        histos_cdf.at(j)->GetXaxis()->SetTitle("Online Calo Jets P_{T}^{4} (Gev)");
        histos_cdf.at(j)->GetYaxis()->SetTitle("(1/N)Cumulative(#DeltaN / 2 GeV)");
    ...
    TLine* l2 = new TLine(10., 0.757, 210., 0.757 );
    l2->SetLineStyle(kDashed);
    l2->SetLineColorAlpha(kGray, 0.7);

    TLine* l3 = new TLine(10., 0.757, 210., 0.757 );
    l3->SetLineStyle(kDashed);
    l3->SetLineColorAlpha(kGray, 0.7);

    TLine* l4 = new TLine(10., 0.757, 210., 0.757 );
    l4->SetLineStyle(kDashed);
    l4->SetLineColorAlpha(kGray, 0.7);

    TLine* l5 = new TLine(10., 0.757, 210., 0.757 );
