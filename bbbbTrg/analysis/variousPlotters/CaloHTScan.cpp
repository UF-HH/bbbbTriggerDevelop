//c++ -o calohtscan CaloHTScan.cpp ../src/Event.cc ../src/TriggerMaker.cc `root-config --cflags --glibs`
...
int main(){
    ...
    for(int j = 0; j < klambdas.size(); j++){
        ...
        histos.at(j)->GetXaxis()->SetTitle("Online Calo Jets HT (Gev)");
        if(j == 0){
            //SM configurations
        ...
        for(int i = 0; i < entries; i++){
            ...
            ev.jetMatch(0.2, "RecoJetsMatched", "CaloJets"); //now we can access PFJets matched
            ...
            if(comp_stop(r_1) > 1 || comp_stop(r_1) < 0){//only events that at least arrive to CaloHT = 320 Cut
                if(ev.GetCaloHT() > 0){
                    histos.at(j)->Fill(ev.GetCaloHT(), ev.weight);
                }  
            }
        ...
        std::string y_title = "(1/N)#DeltaN / 25 GeV";
    ...
    TLatex T2 = TLatex();
    T2.DrawLatexNDC(.66, .84, "#scale[0.7]{#font[42]{#lbar #eta #lbar < 2.4}}");
    c->Draw();
    c->SaveAs("CaloHTMatchedJets.pdf");
    ...
    for(int j = 0; j < klambdas.size(); j++){
        ...
        histos_cdf.at(j)->GetXaxis()->SetTitle("Online Calo Jets HT (Gev)");
    ...
    TLine* l1 = new TLine(320., 0.001, 320., 1.2 );
    l1->SetLineStyle(kDashed);
    l1->SetLineColorAlpha(kGray, 0.7);

    TLine* l2 = new TLine(100., 0.775, 1600., 0.775 );
    l2->SetLineStyle(kDashed);
    l2->SetLineColorAlpha(kGray, 0.7);

    TLine* l3 = new TLine(100., 0.825, 1600., 0.825 );
    l3->SetLineStyle(kDashed);
    l3->SetLineColorAlpha(kGray, 0.7);

    TLine* l4 = new TLine(100., 0.84, 1600., 0.84 );
    l4->SetLineStyle(kDashed);
    l4->SetLineColorAlpha(kGray, 0.7);

    TLine* l5 = new TLine(100., 0.855, 1600., 0.855 );
