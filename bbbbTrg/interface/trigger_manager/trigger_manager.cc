...
std::vector<int> man_filter::compute_or(std::vector<int> results){
    /*
    for(std::vector<int> signle_or_ : OR_connections){
        
        auto min = *std::min_element(signle_or_.begin(), signle_or_.end());
        if(event_count.at(signle_or_.at(0)) >= filter_th.at(signle_or_.at(0)) || event_count.at(signle_or_.at(1)) >= filter_th.at(signle_or_.at(1))){
            results.at(min) = 1;
        }
        
        int scaler = 0;
        for(int i : signle_or_){
...
std::vector<int> man_filter::clear_or(std::vector<int> results){
    /*
    for(auto signle_or_ : OR_connections){
        auto max = *std::max_element(signle_or_.begin(), signle_or_.end());
