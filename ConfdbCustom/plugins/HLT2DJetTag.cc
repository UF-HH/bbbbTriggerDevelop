...
template <typename T>
HLT2DJetTag<T>::HLT2DJetTag(const edm::ParameterSet& config)
    : HLTFilter(config),
      ...
      m_MinBTagBis(config.getParameter<double>("MinBTagBis")),
  ...
  edm::LogInfo("") << " (HLTJetTag) trigger cuts: " << std::endl
                   ...
                   << "\tmin no. tagged jets: " << m_MinBTagBis << "\tTriggerType: " << m_TriggerType << std::endl;
}

template <typename T>
HLT2DJetTag<T>::~HLT2DJetTag() = default;

template <typename T>
void HLT2DJetTag<T>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<double>("MinBTagBis", 0.7);
  desc.add<int>("TriggerType", 0);
  descriptions.add(defaultModuleLabel<HLT2DJetTag<T>>(), desc);
}
...
template <typename T>
bool HLT2DJetTag<T>::hltFilter(edm::Event& event,
  ...
  if(btags_val_.size()>=3){
    std::sort(btags_val_.begin(), btags_val_.end());

    double second = btags_val_.at(btags_val_.size()-2);
    double third = btags_val_.at(btags_val_.size()-3);

    // filter decision
    accept = (second+third >= m_MinBTagBis);
  }
  else accept = (false);
