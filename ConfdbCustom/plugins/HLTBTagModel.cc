...
template <typename T>
HLTBTagModel<T>::HLTBTagModel(const edm::ParameterSet& iConfig)
    : HLTFilter(iConfig),
...
template <typename T>
HLTBTagModel<T>::~HLTBTagModel() = default;


template <typename T>
void HLTBTagModel<T>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  descriptions.add(defaultModuleLabel<HLTBTagModel<T>>(), desc);
}
...
template <typename T>
bool HLTBTagModel<T>::hltFilter(edm::Event& event,
  ...
  for(auto const& btag_jet : *h_JetTags){
    ...
    ++nJet;
    if(nJet < 5){
        inputs_[std::to_string(nJet)+"LeadingBTag"] = btag_jet.second;
    }

    //Save all BTag Scores. Only for central jets eta < 2.5 and pt > 30, that's why the selection on pt and eta
    if ((m_MinTag <= btag_jet.second) && (btag_jet.second <= m_MaxTag) && (abs(btag_jet.first->eta()) <= m_MaxEta) && (abs(btag_jet.first->eta()) >= m_MinEta) &&
        (btag_jet.first->pt() <= m_MaxPt) && (btag_jet.first->pt() >= m_MinPt) )  {
        btags_val_.push_back(btag_jet.second);
        // Store a reference to the jets which passed tagging cuts
        filterproduct.addObject(m_TriggerType, jetRef);
    }
  }

  //If n btag jets < 4 padding with 0...Maybe we should pad with -1 also in training
  if(nJet < 4){
    for(int i = nJet+1; i <=4; i++){
      inputs_[std::to_string(i)+"LeadingBTag"] = 0;
    }
  }
  ...
  if(btags_val_.size() >= 4){
      ...
      for(int j = 0; j < 4-(int)btags_val_.size(); j++){
          FourleadingBTag.push_back(0); //padding at 0 also for maximum btag values in the event...maybe -1?
      }
      for(int i = 0; i < 4; i++){
          inputs_[std::to_string(i+1)+"BTag"] = FourleadingBTag.at(i);
          std::cout << std::endl;
