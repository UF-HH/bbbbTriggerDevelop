...
template <typename T>
HLTBTagSum<T>::HLTBTagSum(const edm::ParameterSet& config)
    : HLTFilter(config),
      ...
      m_MinEta(config.getParameter<double>("MinEta")),
      m_MaxEta(config.getParameter<double>("MaxEta")),
      m_MinPt(config.getParameter<double>("MinPt")),
      m_MaxPt(config.getParameter<double>("MaxPt")),
      m_MinSum(config.getParameter<uint>("SumN")),
      m_MinBTagSum(config.getParameter<double>("MinBTagSum")),
  ...
  edm::LogInfo("") << " (HLTJetTag) trigger cuts: " << std::endl
                   ...
                   << "\tmin no. tagged jets: " << m_MinBTagSum << "\tTriggerType: " << m_TriggerType << std::endl;
}

template <typename T>
HLTBTagSum<T>::~HLTBTagSum() = default;

template <typename T>
void HLTBTagSum<T>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<double>("MinBTagSum", 0.0);
  desc.add<uint>("SumN", 0);
  desc.add<int>("TriggerType", 0);
  descriptions.add(defaultModuleLabel<HLTBTagSum<T>>(), desc);
}
...
template <typename T>
bool HLTBTagSum<T>::hltFilter(edm::Event& event,
  ...
  if(btags_val_.size() >= m_MinSum){
    std::sort(btags_val_.begin(), btags_val_.end());

    double sum = std::accumulate(btags_val_.end() - m_MinSum, btags_val_.end(), 0.0);

    // filter decision
    accept = (sum >= m_MinBTagSum);
  }
  else accept = (false);
