...
template <typename T>
int getObjectType1(const T&) {
  return 0;
}

// specialize for type l1extra::L1EmParticle
template <typename T>
int getObjectType1(const l1extra::L1EmParticle& candidate) {
...
template <typename T>
int getObjectType1(const l1extra::L1EtMissParticle& candidate) {
  switch (candidate.type()) {
...
template <typename T>
int getObjectType1(const l1extra::L1JetParticle& candidate) {
  switch (candidate.type()) {
...
//
template <typename T>
HLTHTQuad<T>::HLTHTQuad(const edm::ParameterSet& iConfig)
    : HLTFilter(iConfig),
  ...
  LogDebug("") << "Input/ptcut/etacut/ncut : " << inputTag_.encode() << " " << min_E_ << " " << min_Pt_ << " "
               << min_Mass_ << " " << max_Mass_ << " " << min_Eta_ << " " << max_Eta_ << " " << min_HT;
}

template <typename T>
HLTHTQuad<T>::~HLTHTQuad() = default;


template <typename T>
void HLTHTQuad<T>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<double>("MinHT", -1.0);
  descriptions.add(defaultModuleLabel<HLTHTQuad<T>>(), desc);
}
...
template <typename T>
bool HLTHTQuad<T>::hltFilter(edm::Event& iEvent,
  ...
  double leading1 = pts.at(pts.size()-1);
  double leading2 = pts.at(pts.size()-2);  
  double leading3 = pts.at(pts.size()-3); 
  double leading4 = pts.at(pts.size()-4); 
  // filter decision
  bool accept( leading1+leading2+leading3+leading4 >= min_HT);
