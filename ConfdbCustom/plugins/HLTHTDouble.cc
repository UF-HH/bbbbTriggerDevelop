...
template <typename T>
HLTHTDouble<T>::HLTHTDouble(const edm::ParameterSet& iConfig)
    : HLTFilter(iConfig),
...
template <typename T>
HLTHTDouble<T>::~HLTHTDouble() = default;


template <typename T>
void HLTHTDouble<T>::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  descriptions.add(defaultModuleLabel<HLTHTDouble<T>>(), desc);
}
...
template <typename T>
bool HLTHTDouble<T>::hltFilter(edm::Event& iEvent,
  ...
  // filter decision
  bool accept( leading1+leading2 >= min_HT);
