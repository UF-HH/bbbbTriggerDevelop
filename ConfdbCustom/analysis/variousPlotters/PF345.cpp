//c++ -o pf345 PF345.cpp ../src/Event.cc ../src/TriggerMaker.cc `root-config --cflags --glibs`
...
int main(){
    ...
    for(int j = 0; j < klambdas.size(); j++){
        ...
        histos.push_back(new TH1F(legState.c_str(), legState.c_str(), 50, 10., 310));
        ...
        histos.at(j)->GetXaxis()->SetTitle("Online Calo Jets P_{T}^{3} (Gev)");
        if(j == 0){
            //SM configurations
        ...
        for(int i = 0; i < entries; i++){
            ...
            if(stop > 5 || stop < 0){//leading jet
                if(PFjets.size() > 2){
                    histos.at(j)->Fill(calo_pts.at(calo_pts.size()-3), ev.weight);
                }
            }
        ...
        std::string y_title = "(1/N)#DeltaN / 6 GeV";
    ...
    for(int j = 0; j < klambdas.size(); j++){
        ...
        stream << std::fixed << std::setprecision(1) << klambdas.at(j);
        std::string legState = "k_{#lambda} = " + stream.str();
        histos_cdf.push_back(new TH1F((legState+std::to_string(j)).c_str(), (legState+std::to_string(j)).c_str(), histos.at(j)->GetNbinsX(), 10., 410));
        ...
        histos_cdf.at(j)->GetXaxis()->SetTitle("Online Calo Jets P_{T}^{3} (Gev)");
        histos_cdf.at(j)->GetYaxis()->SetTitle("(1/N)Cumulative(#DeltaN / 6 GeV)");
    ...
    TLine* l = new TLine(10., 1, 410., 1 );
    l->SetLineStyle(kDashed);
    l->SetLineColorAlpha(kGray, 0.7);

    TLine* l1 = new TLine(45., 0.001, 45., 1.2 );
    l1->SetLineStyle(kDashed);
    l1->SetLineColorAlpha(kGray, 0.7);

    TLine* l2 = new TLine(10., 0.972, 410., 0.972 );
    l2->SetLineStyle(kDashed);
    l2->SetLineColorAlpha(kGray, 0.7);

    TLine* l3 = new TLine(10., 0.972, 410., 0.972 );
    l3->SetLineStyle(kDashed);
    l3->SetLineColorAlpha(kGray, 0.7);

    TLine* l4 = new TLine(10., 0.972, 410., 0.972 );
    l4->SetLineStyle(kDashed);
    l4->SetLineColorAlpha(kGray, 0.7);

    TLine* l5 = new TLine(10., 0.972, 410., 0.972 );
