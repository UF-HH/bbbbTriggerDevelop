//c++ -o tmp MatchEffNewTrig.cpp ../src/TriggerMaker.cc ../src/Event.cc `root-config --cflags --glibs`
#include <iostream>
#include <vector>
#include <string>
#include <algorithm>
...
int main(){
    ...
    TH1F* h_my = new TH1F("h_my", "h_my", 50, 0, 1);
    h_my->SetLineColor(kMagenta);
    TH1F* h_2018 = new TH1F("h_2018", "h_2018", 50, 0, 1);
        ...
        if(myTrigSeq(ev)){
            ...
            if(ev.isResolved(0.8)){
                ...
                if(four_b_match && four_jets_matched){
                    std::vector<double> btags = pfj.btag;
                    std::sort(btags.begin(), btags.end());
                    h_my->Fill(btags.at(btags.size()-4));
                } 
            }
        }
        ...
        if(stop == -1){
            ...
            if(ev.isResolved(0.8)){
                ...
                if(four_b_match && four_jets_matched){
                    std::vector<double> btags = pfj.btag;
                    std::sort(btags.begin(), btags.end());
                    h_2018->Fill(btags.at(btags.size()-4));
                } 
    ...
    leg->SetBorderSize(0);
    leg->AddEntry(h_2018, "#font[42]{2018 Trigger}");
    leg->AddEntry(h_my, "#font[42]{My Trigger}");

    gStyle->SetOptStat(0);

    TCanvas* c2 = new TCanvas("c2", "c2", 1000, 1000, 1000, 700);
    h_my->SetTitle("");
    h_my->GetXaxis()->SetTitle("4th BTag score");
    h_my->GetYaxis()->SetTitle("Events / 0.02");
    h_my->Draw("hist");
