//c++ -o PFpt4scan Pt4PFScan.cpp ../src/Event.cc ../src/TriggerMaker.cc `root-config --cflags --glibs`
...
int main(){
    ...
    for(int j = 0; j < klambdas.size(); j++){
        ...
        histos.push_back(new TH1F(legState.c_str(), legState.c_str(), 40, 10., 210));
        ...
        histos.at(j)->GetXaxis()->SetTitle("Online PF Jets P_{T}^{4} (Gev)");
        if(j == 0){
            //SM configurations
        ...
        for(int i = 0; i < entries; i++){
            ...
            if(comp_stop(r_1) > 0 || comp_stop(r_1) < 0)
                counter++; //L1 check

            std::vector<double> pf_pts;
            for(auto c : PFjets){
                pf_pts.push_back(c->pt);
            }

            if(comp_stop(r_1) > 3 || comp_stop(r_1) < 0){//only events that at least arrive to CaloHT = 320 Cut
                std::sort(pf_pts.begin(), pf_pts.end());
                histos.at(j)->Fill(pf_pts.at(pf_pts.size()-4), ev.weight); //plotting 4th jet pt in order of pt
            }
        ...
        std::string y_title = "(1/N)#DeltaN / 5 GeV";
    ...
    for(int j = 0; j < klambdas.size(); j++){
        ...
        stream << std::fixed << std::setprecision(1) << klambdas.at(j);
        std::string legState = "k_{#lambda} = " + stream.str();
        histos_cdf.push_back(new TH1F((legState+std::to_string(j)).c_str(), (legState+std::to_string(j)).c_str(), histos.at(j)->GetNbinsX(), 10., 210));
        ...
        histos_cdf.at(j)->GetXaxis()->SetTitle("Online PF Jets P_{T}^{4} (Gev)");
        histos_cdf.at(j)->GetYaxis()->SetTitle("(1/N)Cumulative(#DeltaN / 7 GeV)");
    ...
    TLine* l1 = new TLine(30., 0.001, 30., 1.2 );
    l1->SetLineStyle(kDashed);
    l1->SetLineColorAlpha(kGray, 0.7);

    TLine* l2 = new TLine(10., 0.98, 210., 0.98 );
    l2->SetLineStyle(kDashed);
    l2->SetLineColorAlpha(kGray, 0.7);

    TLine* l3 = new TLine(10., 0.98, 210., 0.98 );
    l3->SetLineStyle(kDashed);
    l3->SetLineColorAlpha(kGray, 0.7);

    TLine* l4 = new TLine(10., 0.98, 210., 0.98 );
    l4->SetLineStyle(kDashed);
    l4->SetLineColorAlpha(kGray, 0.7);

    TLine* l5 = new TLine(10., 0.98, 210., 0.98 );
