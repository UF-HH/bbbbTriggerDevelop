//c++ -o tmp PtScan.cpp ../src/Event.cc ../src/TriggerMaker.cc `root-config --cflags --glibs`
...
int main(int argc, char** argv){
    ...
    TFile* f = new TFile("../../../../../../CMSSW_10_2_18/src/bbbbTrg/bbbbTrg/test/filter_tests_2018_nob.root");
    ...
    for(int j = 0; j < klambdas.size(); j++){
        ...
        histos.push_back(new TH1F(legState.c_str(), legState.c_str(), 50, 0, 500));
        ...
        histos.at(j)->GetXaxis()->SetTitle("p_T (H) (Gev)");
        if(j == 0){
            //SM configurations
        ...
        for(int i = 0; i < entries; i++){
            ...
            if(argv[1] == 0){
                histos.at(j)->Fill(Higgs.first.gen_H_pt, ev.weight);
            }
            else{
                histos.at(j)->Fill(Higgs.second.gen_H_pt, ev.weight);
            }
