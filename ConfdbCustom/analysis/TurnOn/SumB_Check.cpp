//c++ -o tmp SumB_Check.cpp ../src/TriggerMaker.cc ../src/Event.cc `root-config --cflags --glibs`
#include <iostream>
#include <vector>
#include <string>
#include <algorithm>
...
int main(){
    ...
    for(int i = 0; i < 4; i++){
        ...
        leg[i]->AddEntry(h_cut[i], "Passed HLT_Quad30_Double60_Sum2LeadingBTag_1p3");
    ...
    for(int i = 0; i < h_cut.size(); i++){
        std::string canv_save_title = ("plots/Consistency_mHH_"+std::to_string(i)+".pdf").c_str();
        c[i]->cd();
        //canv->cd(i+1);
