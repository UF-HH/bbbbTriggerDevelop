//c++ -o tmp TurnOn2018_KinBtag.cpp ../src/TriggerMaker.cc ../src/Event.cc `root-config --cflags --glibs`
#include <iostream>
#include <vector>
#include <string>
#include <algorithm>
...
int main(){
    ...
    std::vector<std::string> path_names_Kin = {"HLT_Quad30_Double60_Sum2LeadingBTag_1p5_v1", "HLT_2018Benchmark_OnlyKin"};
    std::vector<std::string> path_names_Btag = {"HLT_Quad30_Double60_Sum2LeadingBTag_1p5_v1", "HLT_2018Benchmark_OnlyBTag"};
    ...
    TH1F* h_tot_btag = new TH1F("BtagTurnOn", "BtagTurnOn", 15, 0, 1);
    TH1F* h_cut_btag = new TH1F("BtagTurnOn_cut", "BtagTurnOn_cut", 15, 0, 1);
    ...
    for(int ev_idx = 0; ev_idx < event_bits_Btag.size(); ev_idx++){
        ...
        if(evb_b[0] != 0){
            ...
            double btag3 = reco.btag.at(2); //saving thrid btag score offline objects
