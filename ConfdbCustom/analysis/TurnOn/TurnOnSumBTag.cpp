...
int main(){
    ...
    std::vector<std::vector<double>> ranges = {{0,400}, {0,600}, {0,2}, {0,400}, {0,600}, {0,2}};
    int bins = 100;
    ...
    for(int i = 0; i < filters_names.size(); i++){  
        ...
        TEfficiency* eff = new TEfficiency(*h_cut[i], *h_tot[i]);
        eff->SetFillStyle(3004);
        eff->SetLineColor(kRed);

        leg[i]->AddEntry(eff, "Efficiency");
        ...
        h_tot[i]->GetXaxis()->SetTitleSize(.02);
        h_tot[i]->GetXaxis()->SetTitleOffset(2);
        h_tot[i]->GetYaxis()->SetTitle("a.u.");
